...
public class ChormeBrowserScript
{
	public static void main(String [] args) throws InterruptedException, Exception
	{
		...
		{
			writeTofile(driver, dropDown, options.get(i), wordMeaning, i, listNo);
		}
		...
		writer.write(wordMeaning.toString());
        writer.flush();
        writer.close();
        System.out.println("-------------------End--------------");
	}
	
	private static void writeTofile(WebDriver driver, Select dropDown, WebElement option,  StringBuffer wordMeaning, int interationCount, String listNo)
	{
			...
			for(WebElement anchorElement: anchorElements)
			{
				...
				if(meaning == null || "".equals(meaning.trim()) || sentence == null || "".equals(sentence))
				{
					...
					while(true)
					{
						...
						meaning = meaningElement.getText().replaceAll("'", "\\\\'").replace("Meaning: ", "").replace("\"", "\\\"");
						sentence = sentenceElement.getText().replaceAll("'", "\\\\'").replace("Example: ", "").replace("\"", "\\\"");
						if(meaning != null && !"".equals(meaning.trim()) && sentence != null && !"".equals(sentence))
						{
				...
				wordMeaning.append("\""+anchorElement.getText()+"\",\""+meaning+"\",\""+sentence+"\",\""+anchorElement.getText()+"\",\""+option.getText()+"\",\""+listNo+"\",");
